<x-layouts::app :title="__('Tambah Nasabah')">
    <div class="mx-auto max-w-3xl space-y-6">
        <header class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ __('Tambah Nasabah') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Tambahkan nasabah baru ke sistem.</p>
            </div>
        ...
        <form action="{{ route('customers.store') }}" method="POST" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            @csrf
