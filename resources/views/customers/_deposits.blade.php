...
<section class="mt-6 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
    <div class="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-neutral-800">
        <div class="flex items-center gap-2.5">
            ...
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">Setoran</h3>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2">
            <form method="GET" class="flex items-center gap-2">
                <div class="relative">
                    ...
                    <input name="deposit_search" value="{{ request('deposit_search') }}" placeholder="Cari transaksi"
                ...
                <select name="deposit_sort"
                    ...
                    <option value="transaction_date_desc" {{ request('deposit_sort') === 'transaction_date_desc' ? 'selected' : '' }}>Terbaru</option>
                    <option value="transaction_date_asc" {{ request('deposit_sort') === 'transaction_date_asc' ? 'selected' : '' }}>Terlama</option>
                </select>
                <button type="submit"
            ...
            @endphp
            <a href="{{ route('deposit-transactions.create') }}{{ $currentCustomerId ? '?customer_id=' . $currentCustomerId : '' }}"
    ...
    @if($deposits->isEmpty())
        <div class="flex flex-col items-center justify-center gap-2 px-5 py-12 text-center">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Belum ada setoran untuk nasabah ini.</p>
        </div>
    @else
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-neutral-800">
                    @foreach($deposits as $d)
                        <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-neutral-800/60">
                            <td class="px-5 py-3 text-gray-500 dark:text-gray-400">{{ $d->id }}</td>
                            <td class="px-5 py-3 font-medium text-gray-900 dark:text-white">{{ $d->transaction_number }}</td>
                            <td class="px-5 py-3 text-gray-600 dark:text-gray-300">{{ $d->transaction_date->format('d M Y') }}</td>
                            <td class="px-5 py-3 text-right font-semibold text-emerald-600 dark:text-emerald-400">Rp{{ number_format($d->total_amount, 2) }}</td>
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('deposit-transactions.show', $d) }}{{ $currentCustomerId ? '?customer_id=' . $currentCustomerId : '' }}" title="Lihat"
                                    ...
                                    <a href="{{ route('deposit-transactions.edit', $d) }}{{ $currentCustomerId ? '?customer_id=' . $currentCustomerId : '' }}" title="Ubah"
                                    ...
                                    <form action="{{ route('deposit-transactions.destroy', $d) }}{{ $currentCustomerId ? '?customer_id=' . $currentCustomerId : '' }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('Apakah Anda yakin ingin menghapus transaksi ini?') }}')">
                                        @csrf
                                        @method('DELETE')
