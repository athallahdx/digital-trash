<x-layouts::app :title="__('Riwayat Penarikan')">
    <div class="flex flex-col gap-6">
        <header class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ __('Riwayat Penarikan') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Melacak penarikan keluar dan jumlahnya.</p>
            </div>

            <div class="flex items-center gap-3">
                <form method="GET" action="{{ route('withdrawal-transactions.index') }}" class="flex items-center gap-2">
                ...
                <a href="{{ route('withdrawal-transactions.create') }}"
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-neutral-800/60">
                        <tr class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                            ...
                            <th class="px-5 py-3 text-right"> <a href="{{ request()->fullUrlWithQuery(['sort' => 'amount', 'direction' => (request('sort') == 'amount' && request('direction') == 'desc') ? 'asc' : 'desc', 'page' => 1]) }}">Total</a></th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-neutral-800">
                        @forelse($transactions as $transaction)
                            <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-neutral-800/60">
                                ...
                                <td class="px-5 py-3.5 text-right font-semibold text-rose-600 dark:text-rose-400">{{ 'Rp ' . number_format($transaction->amount ?? 0, 2, ',', '.') }}</td>
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('withdrawal-transactions.show', $transaction) }}" title="Lihat"
                                        ...
                                        <a href="{{ route('withdrawal-transactions.edit', $transaction) }}" title="Ubah"
                                        ...
                                        <form action="{{ route('withdrawal-transactions.destroy', $transaction) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('Apakah Anda yakin ingin menghapus transaksi ini?') }}')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-14">
                                    <div class="flex flex-col items-center gap-3 text-center">
                                        ...
                                        <div class="text-sm font-semibold text-gray-900 dark:text-white">Belum ada penarikan</div>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">Buat penarikan untuk mulai mencatat pengeluaran.</p>
                                        <a href="{{ route('withdrawal-transactions.create') }}"
