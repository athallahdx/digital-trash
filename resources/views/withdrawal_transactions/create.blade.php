<x-layouts::app :title="__('Buat Penarikan')">
    <div class="mx-auto max-w-3xl space-y-6">
        <header class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ __('Buat Penarikan') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Catat penarikan baru untuk nasabah.</p>
            </div>
        ...
        <form action="{{ route('withdrawal-transactions.store') }}{{ isset($selectedCustomerId) && $selectedCustomerId ? '?customer_id=' . $selectedCustomerId : '' }}" method="POST" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            @csrf

            @include('withdrawal_transactions._form', ['customers' => $customers, 'selectedCustomerId' => $selectedCustomerId ?? null])
        </form>
    </div>
</x-layouts::app>