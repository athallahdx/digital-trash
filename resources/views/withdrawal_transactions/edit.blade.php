<x-layouts::app :title="__('Ubah Penarikan')">
    <div class="mx-auto max-w-3xl space-y-6">
        <header class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ __('Ubah Penarikan') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Ubah detail penarikan.</p>
            </div>
            <a href="{{ request('customer_id') ? route('customers.show', request('customer_id')) : route('withdrawal-transactions.index') }}"
        ...
        <form action="{{ route('withdrawal-transactions.update', $transaction) }}{{ isset($selectedCustomerId) && $selectedCustomerId ? '?customer_id=' . $selectedCustomerId : '' }}" method="POST" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            @csrf
            @method('PUT')

            @include('withdrawal_transactions._form', ['transaction' => $transaction, 'customers' => $customers, 'selectedCustomerId' => $selectedCustomerId ?? null])
        </form>
    </div>
</x-layouts::app>