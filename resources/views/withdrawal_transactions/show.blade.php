<x-layouts::app :title="__('Detail Penarikan')">
    <div class="mx-auto max-w-3xl space-y-6">
        <header class="flex items-center justify-between gap-4">
            <div class="min-w-0">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Detail transaksi penarikan ini.</p>
            </div>

            <div class="flex shrink-0 items-center gap-2">
                <a href="{{ route('withdrawal-transactions.edit', $transaction) }}{{ request('customer_id') ? '?customer_id=' . request('customer_id') : '' }}"
                ...
                <form action="{{ route('withdrawal-transactions.destroy', $transaction) }}{{ request('customer_id') ? '?customer_id=' . request('customer_id') : '' }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('Apakah Anda yakin ingin menghapus transaksi ini?') }}')">
                    @csrf
                    @method('DELETE')
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            <dl class="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <dd class="mt-1.5 text-sm font-semibold text-rose-600 dark:text-rose-400">Rp{{ number_format($transaction->amount, 2) }}</dd>
                </div>
